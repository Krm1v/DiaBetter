...
extension UIElementsBuilder {
    ...
    func buildBackButton(with title: String) -> UIButton {
        ...
        button.titleLabel?.font = FontFamily.Montserrat.semiBold.font(
            size: Constants.basicButtonTitleFontSize)
        return button
    }
    
    func buildSystemButton(
        with title: String = "",
        fontSize: CGFloat = 17
    ) -> UIButton {
        let button = UIButton(type: .system)
    ...
    func buildUserInfoLabel(with text: String = "") -> UILabel {
        ...
        label.font = FontFamily.Montserrat.semiBold.font(
            size: Constants.basicLabelFontSize)
        label.text = text
