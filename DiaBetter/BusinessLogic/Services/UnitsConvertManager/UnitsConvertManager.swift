...
protocol UnitsConvertManager: AnyObject {
    ...
    func convertValueIfNeeded(
        _ value: inout Decimal,
        from initialUnit: SettingsUnits.GlucoseUnitsState,
        to targetUnit: SettingsUnits.GlucoseUnitsState
    )
...
extension UnitsConvertManagerImpl: UnitsConvertManager {
    func convertValueIfNeeded(
        ...
        if initialUnit == .mmolL && targetUnit == .mgDl {
            value *= Constants.mgDlConversionFactor
        } else if initialUnit == .mgDl && targetUnit == .mmolL {
            value /= Constants.mgDlConversionFactor
        }
    }
    ...
    func setToDefaultValue(glucoseValue: Decimal?) -> AnyPublisher<Decimal?, Never> {
        ...
        return settingsService.settingsPublisher
            .map { settings in
                ...
                case .mgDl:
                    if let glucoseValue {
                        updatedGlucose = glucoseValue / Constants.mgDlConversionFactor
                    }
                    return updatedGlucose
                }
            }
    ...
    func setToDefaultValue(carbs: Decimal?) -> AnyPublisher<Decimal?, Never> {
        ...
        return settingsService.settingsPublisher
            .map { settings in
                ...
                case .grams:
                    if let carbs {
                        updatedCarbs = carbs / Constants.gramsConversionFactor
                    }
                    return updatedCarbs
                }
            }
