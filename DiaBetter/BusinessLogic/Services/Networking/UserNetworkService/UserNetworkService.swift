...
extension UserNetworkServiceImpl: UserNetworkService {
    func updateUser(
        ...
        objectId: String
    ) -> AnyPublisher<UserResponseModel, NetworkError> {
        networkProvider.execute(
            endpoint: .update(user: user, id: objectId),
            decodeType: UserResponseModel.self)
    }
    ...
    func uploadUserProfilePhoto(
        ...
        userId: String
    ) -> AnyPublisher<UserProfilePictureResponse, NetworkError> {
