...
final class DiaryDetailSceneView: BaseView {
    ...
    private lazy var viewStack = buildStackView(
        axis: .vertical,
        alignment: .fill,
        distribution: .fill,
        spacing: Constants.stackViewMinSpacing)
    
    private lazy var buttonsStack = buildStackView(
        axis: .vertical,
        alignment: .fill,
...
private extension DiaryDetailSceneView {
    func setupUI() {
        ...
        deleteButton.setTitleColor(
            Colors.customRed.color,
            for: .normal)
        
        noteView.layer.borderWidth = Constants.defaultBorderWidth
        noteView.layer.borderColor = UIColor.white.cgColor
        noteView.rounded(Constants.defaultCornerRadius)
    }
    ...
    func setupLayout() {
        ...
        axisScrollView.contentView.addSubview(
            ...
            constraints: [
                ...
                vStack.bottomAnchor.constraint(
                    ...
                    constant: -Constants.largeStackViewInset)
            ])
        ...
        [datePicker, glucoseView, mealView, fastInsulinView, basalInsulinView].forEach {
            $0.heightAnchor.constraint(equalToConstant: Constants.defaultViewHeight)
                .isActive = true
            viewStack.addArrangedSubview($0) }
        
        noteView.heightAnchor.constraint(equalToConstant: Constants.noteViewHeight)
            .isActive = true
...
private enum Constants {
    ...
    static let largeStackViewInset:           CGFloat = 20
    static let defaultViewHeight:             CGFloat = 44
    static let noteViewHeight:                CGFloat = 160
