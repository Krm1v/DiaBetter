...
private extension DiarySceneView {
	...
	func makeSectionHeader() -> NSCollectionLayoutBoundarySupplementaryItem {
		let headerSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(Constants.defaultItemSize),
	...
	func setupDatasource() {
		datasource = UICollectionViewDiffableDataSource<DiarySceneSection, DiarySceneItem>(collectionView: collectionView, cellProvider: { (collectionView, indexPath, item) -> UICollectionViewCell? in
			switch item {
			case .record(let model):
				let cell = collectionView.configureCell(cellType: RecordCell.self,
														indexPath: indexPath)
		...
		datasource?.supplementaryViewProvider = { [weak self] (collectionView, kind, indexPath) in
			...
			let sectionHeader = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: MainSectionHeader.reuseId, for: indexPath) as? MainSectionHeader
