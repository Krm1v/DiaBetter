...
private extension HomeSceneView {
	...
	func setupCollection() {
		collectionView.register(LineChartCell.self,
								forCellWithReuseIdentifier: LineChartCell.reuseID)
		collectionView.register(CubicLineChartCell.self,
								forCellWithReuseIdentifier: CubicLineChartCell.reuseID)
		collectionView.register(InsulinUsageChartCell.self,
	...
	func makeWidgetSection() -> NSCollectionLayoutSection {
		let itemSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(Constants.defaultWidgetItemWidth),
											  heightDimension: .fractionalHeight(Constants.defaultWidgetItemHeight))
		let item = NSCollectionLayoutItem(layoutSize: itemSize)
		item.contentInsets = NSDirectionalEdgeInsets(top: Constants.defaultEdgeInsets,
													 leading: Constants.defaultEdgeInsets,
													 bottom: .zero,
													 trailing: Constants.defaultEdgeInsets)
		let groupSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(Constants.defaultWidgetItemWidth),
	...
	func setupDatasource() {
		datasource = UICollectionViewDiffableDataSource<ChartSection, ChartsItems>(collectionView: collectionView, cellProvider: { (collectionView, indexPath, item) -> UICollectionViewCell? in
			switch item {
			case .lineChart(let model):
				let cell = collectionView.configureCell(cellType: LineChartCell.self,
														indexPath: indexPath)
			...
			case .cubicLineChart(let model):
				let cell = collectionView.configureCell(cellType: CubicLineChartCell.self,
														indexPath: indexPath)
				cell.configure(with: model)
				return cell
			case .insulinUsage(let model):
				let cell = collectionView.configureCell(cellType: InsulinUsageChartCell.self,
														indexPath: indexPath)
				cell.configure(with: model)
				return cell
			}
