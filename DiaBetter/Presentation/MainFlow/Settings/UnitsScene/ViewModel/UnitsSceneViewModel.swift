...
final class UnitsSceneViewModel: BaseViewModel {
	...
	@Published var sections: [SectionModel<UnitsSceneSections, UnitsSceneItems>] = []
	...
	//MARK: - Public methods
	func saveSettings() {
		appSettingsService.settings = AppSettingsModel(notifications: appSettingsService.settings.notifications,
	...
	func glucoseTargetDidChanged(_ target: MinMaxGlucoseTarget, _ value: Double) {
		switch target {
		case .min:
...
private extension UnitsSceneViewModel {
	func updateDatasource() {
		...
		let glucoseTargetSectionModel = UnitsSectionModel(title: Localization.targetGlucose)
		let glucoseUnitsModel = GlucoseUnitsCellModel(title: Localization.glucoseUnits, currentUnit: glucoseUnit)
		let carbsUnitsModel = CarbsUnitsCellModel(title: Localization.carbohydrates, currentUnit: carbohydrates)
		
		let mainSection = SectionModel<UnitsSceneSections, UnitsSceneItems>(
		...
		let glucoseTargetSection = SectionModel<UnitsSceneSections, UnitsSceneItems>(
