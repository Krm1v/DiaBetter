...
private extension UnitsSceneView {
	...
	func setupCollectionView() {
		collectionView.register(GlucoseUnitsCell.self,
								forCellWithReuseIdentifier: GlucoseUnitsCell.reuseID)
		collectionView.register(CarbsUnitsCell.self,
								forCellWithReuseIdentifier: CarbsUnitsCell.reuseID)
		collectionView.register(TargetGlucoseCell.self,
								forCellWithReuseIdentifier: TargetGlucoseCell.reuseID)
		collectionView.register(UnitsSectionHeader.self,
	...
	func setupDatasource() {
		datasource = UICollectionViewDiffableDataSource(collectionView: collectionView, cellProvider: { [weak self] (collectionView, indexPath, item) -> UICollectionViewCell? in
			...
			case .plainWithSegmentedControl(let model):
				let cell = collectionView.configureCell(cellType: GlucoseUnitsCell.self,
														indexPath: indexPath)
			...
			case .plainWithUIMenu(let model):
				let cell = collectionView.configureCell(cellType: CarbsUnitsCell.self, indexPath: indexPath)
				cell.configure(model)
				cell.actionPublisher
			...
			case .plainWithStepper(let model):
				let cell = collectionView.configureCell(cellType: TargetGlucoseCell.self, indexPath: indexPath)
				cell.configure(model)
				cell.actionPublisher
					.sink { [unowned self] action in
						switch action {
						case .stepperDidTapped(let value):
							guard let object = self.getObject(with: indexPath) else { return }
		...
		datasource?.supplementaryViewProvider = { [weak self] (collectionView, kind, indexPath) -> UICollectionReusableView? in
			...
			let sectionHeader = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: UnitsSectionHeader.reuseId, for: indexPath) as? UnitsSectionHeader
			let section = self?.datasource?.snapshot().sectionIdentifiers[indexPath.section]
			if section?.title == nil { return nil }
	...
	func makeMainListSection(with layoutEnvironment: NSCollectionLayoutEnvironment) -> NSCollectionLayoutSection {
	...
	func makeSectionHeader() -> NSCollectionLayoutBoundarySupplementaryItem {
		...
		let sectionHeader = NSCollectionLayoutBoundarySupplementaryItem(layoutSize: headerSize, elementKind: UICollectionView.elementKindSectionHeader, alignment: .topLeading, absoluteOffset: CGPoint(x: .zero, y: 10))
		return sectionHeader
	}
