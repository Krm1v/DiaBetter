...
private extension BackupSceneView {
	...
	func setupCollectionView() {
		collectionView.register(BackupDateCell.self,
								forCellWithReuseIdentifier: BackupDateCell.reuseID)
		collectionView.register(BackupPlainCell.self,
								forCellWithReuseIdentifier: BackupPlainCell.reuseID)
		collectionView.register(BackupSectionFooter.self,
	...
	func setupDatasource() {
		datasource = BackupSceneDatasource(collectionView: collectionView, cellProvider: { [weak self] (collectionView, indexPath, item) -> UICollectionViewCell? in
			guard let self = self else { return UICollectionViewCell() }
			
			switch item {
			case .datePickerItem(let model):
				let cell = collectionView.configureCell(cellType: BackupDateCell.self,
														indexPath: indexPath)
				...
				cell.actionPublisher
					.sink { [unowned self] actions in
						...
						case .datePickerValueDidChanged(let date):
							guard let object: BackupDateCellModel = self.getObject(indexPath) else { return }
			...
			case .plainItem(let model):
				let cell = collectionView.configureCell(cellType: BackupPlainCell.self,
														indexPath: indexPath)
				cell.configure(model)
				return cell
			}
	...
	func bindActions() {
		collectionView.didSelectItemPublisher
			.sink { [unowned self] indexPath in
				guard let object: BackupShareCellModel = self.getObject(indexPath) else { return }
